 ...
 const handleDelete = (index) => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 pt-16">
      <div className="w-full max-w-md rounded-2xl border border-gray-200
        ...
        <div className="rounded-xl border border-gray-200 bg-[rgba(255,255,255,0.1)] p-4 flex flex-col gap-2">
          {news.img && (
            <img src={news.img} className="w-full h-32 object-cover rounded-lg" />
          )}
          <p className="text-white font-bold text-sm">{news.head}</p>
          <p className="text-white text-xs">{news.date} · {news.time}</p>
        </div>
        ...
        <div className="flex gap-4">
          ...
          <button
            ...
            className="flex-1 flex items-center justify-center gap-2 py-3 rounded-2xl
              ...
              transition-all duration-200 hover:scale-105 active:scale-95 hover:bg-[rgba(255,0,0,0.35)]"
          >
